...
function mostrarNotificacao() {
  // Remove a verificação do localStorage para aparecer sempre
  const notificacao = document.querySelector(".cabelo.notificacao");
  if (notificacao) {
    notificacao.style.display = "flex";

    // Esconde após 5.3 segundos (tempo da animação)
    setTimeout(() => {
...
document.addEventListener("DOMContentLoaded", function () {
  ...
  if (botaoFechar) {
    botaoFechar.addEventListener("click", function () {
      ...
      if (notificacao) {
        notificacao.style.display = "none";
        // Remove o localStorage.setItem para não bloquear futuras exibições
      }
    });
  }
});
...
const indicadores = indicadoresContainer.querySelectorAll(
  ".carrossel-indicador"
);
